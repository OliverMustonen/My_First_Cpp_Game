...
simulate_game(Input * input, float dt) {
	...
	float player_1_ddp = 0.f; // units per seconds
	if(enemy_is_ai) {
	...
	float player_2_ddp = 0.f; // units per seconds
	...
	draw_rect(-80, player_2_p, player_half_size_x, player_half_size_y, 0xff0000);
		...
		if (hot_button == 0) {
			draw_text("SINGEL PLAYER", -80, -10, 1, 0xff0000);
			draw_text("MULTIPLAYER", 20, -10, 1, 0xaaaaaa);

		} else {
			draw_text("SINGEL PLAYER", -80, -10, 1, 0xaaaaaa);
			draw_text("MULTIPLAYER", 20, -10, 1, 0xff0000);
		}

		draw_text("PONG TUTORIAL", -30, 40, 1, 0xffffff);
